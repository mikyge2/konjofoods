  ...
  return (
    <div className="min-h-screen">
      ...
      <section className="py-16 sm:py-20 md:py-28 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-12 sm:mb-16 md:mb-20">
            <span className="inline-block px-4 py-2 bg-brand-gold/20 text-brand-black font-semibold text-sm rounded-full mb-4">
              HANDPICKED COLLECTION
            </span>
            <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-brand-black mb-6">
              Our Signature <span className="text-brand-red">Spices</span>
            </h2>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto px-4">
              Discover the authentic flavors that define Ethiopian cuisine, crafted with care and tradition
            </p>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-10">
            {featuredProducts.map((product, index) => (
              <Card key={product.id} className="border-none shadow-xl overflow-hidden group hover:shadow-2xl transition-all duration-500 hover:-translate-y-3 bg-white rounded-3xl"
                style={{ animationDelay: `${index * 0.1}s` }}>
                <div className="h-72 sm:h-80 overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 relative">
                  <img
                    src={product.image}
                    alt={product.name}
                    loading="lazy"
          ...
          <div className="text-center mt-12 sm:mt-16 px-4">
            <Link to="/products">
              <Button size="lg" className="bg-brand-red hover:bg-brand-dark-red text-white px-10 py-7 text-lg font-semibold group shadow-2xl hover:shadow-brand-red/50 transition-all rounded-full">
                View All Products
                <ArrowRight className="ml-2 group-hover:translate-x-2 transition-transform" size={22} />
              </Button>
            </Link>
